...
<template>
  <div class="InnerDiv">
    <div >
      ...
      <label  >报名多选题 - {{ QesData.question }}</label>
    ...
    <el-checkbox-group v-model="QesData.radio" class="InnerDiv">
      <el-row :gutter="10" class="Choice" type="flex" align="top" justify="left"  v-for="(choice,i) in QesData.choices" >
        <el-checkbox v-if="QesData.Times[i]===0" disabled :label="QesData.choices[i]" :key="choice"
                     class="Choice"
        >   </el-checkbox>
        <el-checkbox v-else :label="QesData.choices[i]" :key="choice"
                     class="Choice"
        >
        </el-checkbox>
