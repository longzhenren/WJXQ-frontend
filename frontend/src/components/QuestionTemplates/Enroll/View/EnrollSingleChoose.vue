...
<template>
  ...
  <div  class="InnerDiv" >
    <div >
      ...
      <label  >报名单选题- {{ QesData.question }}</label>
    ...
    <el-radio-group v-model="QesData.radio" class="InnerDiv" style="width: 100%" >
      <el-row :gutter="10" class="Choice" type="flex" align="top" justify="left"  v-for="(choice,i) in QesData.choices" >
        <el-col >
          <el-radio  v-if="QesData.Times[i]===0" disabled class="Choice" :label="QesData.choices[i]" ></el-radio>
          <el-radio  v-else class="Choice" :label="QesData.choices[i]" >
          </el-radio>
          <label style="font-size: 12px;color: #6E6E6E" >剩余:{{QesData.Times[i]}}</label>
        </el-col>
